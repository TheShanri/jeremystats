...
# Determine all sessions, add the missing session, and sort them
...
for label in labels_present:
    ...
    ax1.scatter(
        ...
        subset["y_val"], 
        label=label,
        color=color_map.get(label, "black"), # Enforce hardcoded color
        alpha=0.8, 
        edgecolors='k',
        s=50
    )
...
# Apply colors and styles to primary Y-axis labels
...
ax1.set_title("Events over Time per Session")

# Adjust legend placement to avoid overlapping the secondary Y-axis
