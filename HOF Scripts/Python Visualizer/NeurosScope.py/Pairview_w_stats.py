...
class ComboMaster:
    ...
    def scan_stats_database(self):
        ...
        # Regex map: extracts Mouse_Session, EventType, Trial, PairA, PairB
        ...
        for root, _, files in os.walk(STATS_DIR):
            for file in files:
                ...
                if match:
                    ...
                    trial_num = str(int(match.group(3))) # Normalize '03' to '3'
                    ch1 = match.group(4)
                    ch2 = match.group(5)
                    
                    # Deconstruct session ID back to database standard
    ...
    def setup_ui(self):
        ...
        # Network Stats Button Integration
        stats_btn = tk.Button(self.timeline_frame, text="COMPARE NETWORK STATS", 
                              command=self.open_stats_viewer, bg="#0055ff", fg="white", font=("Arial", 9, "bold"))
        stats_btn.pack(side=tk.RIGHT, padx=15, pady=10)
    ...
    def open_stats_viewer(self):
        ...
        pairs = self.stats_db.get(key, [])
        
        if not pairs:
            print(f"[UI] No dynamic stats available yet for: {mouse} | {sess} | {event} | #{eidx}")
            return
            
        # Launch dedicated scrollable rendering window
        win = tk.Toplevel(self.root)
        win.title(f"Dynamic Connectivity Tensors | {event} #{eidx} | {mouse}_{sess}")
        ...
        win.photos = [] # Cache lock
        
        row_frame = None
        cols = 3
        
        for i, (ch1, ch2, path) in enumerate(pairs):
            if i % cols == 0:
                row_frame = tk.Frame(scrollable_frame, bg="#111")
                row_frame.pack(fill=tk.X, pady=10, padx=10)
                
            card = tk.Frame(row_frame, bg="black", bd=2, relief=tk.RIDGE)
            card.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=5)
            
            lbl = tk.Label(card, text=f"{ch1} vs {ch2}", bg="#222", fg="#00ffcc", font=("Arial", 9, "bold"))
            lbl.pack(side=tk.TOP, fill=tk.X)
            
            img_cnv = tk.Canvas(card, bg="black", width=420, height=280, highlightthickness=0)
            img_cnv.pack(side=tk.TOP, fill=tk.BOTH, expand=True)
            img_cnv.bind("<Button-1>", lambda e, p=path: self.open_image_viewer(p)) # Click to expand
            
            try:
                img = Image.open(path)
                img.thumbnail((420, 280), Image.Resampling.LANCZOS)
                ph = ImageTk.PhotoImage(img)
                img_cnv.create_image(210, 140, image=ph, anchor=tk.CENTER)
                win.photos.append(ph)
            except Exception:
                img_cnv.create_text(210, 140, text="RENDER FAILED", fill="red", font=("Arial", 12, "bold"))
