function []=create_kilosort_binary (filedir, xlsxfilename, probeName)
% Author: Rain Younger skyler.younger@uvm.edu (need to update this email
% eventually)
% ⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣀⣤⣄⣀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
% ⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣠⣾⣿⣿⣿⣿⣿⣿⣦⡀⠀⠀⠀⠀⠀⠀⠀
% ⠀⠀⠀⠀⠀⣠⣾⣿⣿⣶⣄⠀⣸⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⡆⠀⠀⠀⠀⠀⠀
...
%   function. H3 probes are 'CSC'

cd (filedir); % Set current directory
% read excel feedersheet
[data,txt] =  xlsread(xlsxfilename);
...
for feeder_index = 1:size(data,1);
    ...
    if isdir(path_output) == true;
        ...
        binary_feeder_index = feeder_index + 1;
        binary_feeder(binary_feeder_index) = convertCharsToStrings(path_output);
        binary_filler(binary_feeder_index) = "1";
