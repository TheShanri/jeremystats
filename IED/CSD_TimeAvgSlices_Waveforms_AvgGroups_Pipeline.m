...
outSOL = fullfile(outRoot, "CSD_TimeAvg_SOLID.png");
outSPU = fullfile(outRoot, "CSD_TimeAvg_SPUTTER.png");
...
% ---------- Build & render ----------
[statsSOL, okSOL] = buildAndRender(evtSOL, 'SOLID', outSOL);
[statsSPU, okSPU] = buildAndRender(evtSPU, 'SPUTTER', outSPU);

% ---------- Save stats CSV ----------
res = struct('pngSolid',"",'pngSputter',"",'statsCSV',"");
try
    S = [statsSOL; statsSPU];
    if ~isempty(S)
...
if okSOL, res.pngSolid   = outSOL; end
if okSPU, res.pngSputter = outSPU; end
end

% ============================= HELPERS =============================

function [Tstats, ok] = buildAndRender(evtList, tag, outPath)
